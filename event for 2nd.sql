...
CONSOLIDATION_REP_COMMON:BEGIN
  ...
  SET vMIN_OPERATION_DT = (SELECT MIN(T.DT) FROM work.operations T);
  ...
  INSERT INTO common_report_consolidation
  (
    REPORT_DATE,
    ID_COUNTRY,
    ID_TYPE_OPER,
  ...
  SELECT IF(T.NAME_COUNTRY IS NULL,
         ...
         T.NAME_OPER,
         SUM(TOTAL) AS TOTAL,
         SUM(TOTAL_DBT) AS TOTAL_DBT,
         SUM(TOTAL_KRD) AS TOTAL_KRD,
         SUM(TOTAL_COMMISSION) AS TOTAL_COMMISSION,
         SUM(TOTAL_ITOG) AS TOTAL_ITOG,
         SUM(TOTAL_ITOG_DBT) AS TOTAL_ITOG_DBT,
         SUM(TOTAL_ITOG_KRD) AS TOTAL_ITOG_KRD 
  FROM tt_prepare_report T
  ...
  --        OT.ID_TYPE_OPER,
  --        O.AMOUNT_OPER,
  --        @COMISSION:=CAST(IFNULL(OT.COMISSION, 0) * 100 AS UNSIGNED),
  --        O.AMOUNT_OPER * @COMISSION AS COMISSION,
  -- 
  -- 
  -- 
  -- FROM work.operations    O
  -- JOIN work.type_opers   OT ON OT.ID_TYPE_OPER = O.ID_TYPE_OPER
  -- JOIN work.users         U ON U.ID_USER = O.ID_USER
  -- JOIN work.currencies    C ON C.ID_CURRENCY = U.ID_CURRENCY
  -- WHERE O.DT BETWEEN iDATETIME_FROM AND vDATE_FROM
  --    OR O.DT BETWEEN vDATE_TO + INTERVAL 1 DAY AND iDATETIME_TO
 ...
 -- DROP TEMPORARY TABLE IF EXISTS tt_consolid;
 -- CREATE TEMPORARY TABLE tt_consolid AS
 -- SELECT *
