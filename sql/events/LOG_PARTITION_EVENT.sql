...
LOG_PARTITION_EVENT:BEGIN
    ...
    IF iMODE = 'create' THEN
      ...
      SET @SQL_PARTITION = CONCAT('
          ...
          @SQL_PARTITION,'
          PARTITION MVALUE VALUES LESS THAN (MAXVALUE)
          );'
      );
    ...
    ELSEIF iMODE = 'modify' THEN
      ...
      IF @CREATE_PARTITIONS IS NOT NULL THEN
          SET @SQLS_CREATE = CONCAT('
          ALTER TABLE log_check_user REORGANIZE PARTITION MVALUE INTO (',
          @CREATE_PARTITIONS, ',
          PARTITION MVALUE VALUES LESS THAN (MAXVALUE)
          )');
...
DROP EVENT IF EXISTS LOG_PARTITIONING $$
CREATE EVENT LOG_PARTITIONING
ON SCHEDULE EVERY 1 DAY STARTS '2020-01-31 01:00:00'
ON COMPLETION NOT PRESERVE
ENABLE
COMMENT '' DO
BEGIN
  CALL work.LOG_PARTITION_EVENT('modify');
END $$
